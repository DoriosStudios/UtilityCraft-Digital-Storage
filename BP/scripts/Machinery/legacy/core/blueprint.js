...
export function buildBlueprintLore(data) {
  ...
  const lore = [`\u00A7r\u00A77 Recipe: \u00A7r\u00A7f${formatItemId(resultId)}`, "\u00A7r\u00A77 Materials:"];
...
export function writeBlueprintData(itemStack, data, options = {}) {
  ...
  const materialsSource = Object.prototype.hasOwnProperty.call(data, "materials") ? data.materials : current.materials;
  const materials = normalizeMaterials(materialsSource);
  const leftoverSource = Object.prototype.hasOwnProperty.call(data, "leftover") ? data.leftover : current.leftover;
  ...
  if (options.setLore !== false) {
    const lore = Array.isArray(data.lore) ? data.lore.filter((line) => typeof line === "string") : buildBlueprintLore({ id, materials });
